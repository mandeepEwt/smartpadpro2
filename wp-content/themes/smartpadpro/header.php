...
.top-book-demo
{
		...
		<div class="headerTop bg-dark col-lg-12" >
            <div class="col-lg-12">                              
                ...
                <div class="show_flags" style="position : relative; float:right; width: 80px; cursor: pointer; padding: 9px 0;">
					<span> 
						<img src="/wp-content/themes/smartpadpro/assets/images/flags/aus.png" style="border : 2px solid white;">
    ...
    <?php if(isset($_POST["post_pricing_values"]) ){
		...
		$_SESSION["optional_team_messaging"]    = $_POST["optional_team_messaging"]; 
		$_SESSION["optional_client_links"]      = $_POST["optional_client_links"];
		$_SESSION["optional_send_reports"]      = $_POST["optional_send_reports"];
		$_SESSION["optional_email_marketing"]   = $_POST["optional_email_marketing"];
		$_SESSION["show_default"]               = $_POST["show_default"];
	}		
		  ...
		  var total_amount;
			<?php if(isset($_POST["post_pricing_values"]) ){  ?> 
				if( jQuery("#count-users-hidden").length>0 ){
					...
					jQuery("#optional_team_messaging-hidden").val(<?php echo $_SESSION["optional_team_messaging"]; ?>);
					jQuery("#optional_client_links-hidden").val(<?php echo $_SESSION["optional_client_links"]; ?>);
					jQuery("#optional_send_reports-hidden").val(<?php echo $_SESSION["optional_send_reports"]; ?>);
					jQuery("#optional_email_marketing-hidden").val(<?php echo $_SESSION["optional_email_marketing"]; ?>);
					jQuery("#total_amount-hidden").val("<?php echo $_SESSION["pricing_amount"]; ?>");
			...
			<?php } else
			{
				...
				?>
				var my_amount = jQuery("#total_amount span").text();
				jQuery("#total_amount-hidden").val(my_amount);
				<?php
			}?> 
