<?php namespace App\PostTypes\Core\CustomMetaBox;
...
class cmb_Meta_Box {
	...
	function __construct( $meta_box ) {
		...
		$show_filters = '\App\PostTypes\Core\CustomMetaBox\helpers\cmb_Meta_Box_Show_Filters';
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		foreach ( $meta_box['fields'] as $field_args ) {
			...
			if ( 'group' == $field_args['type'] ) {
				...
				$field = new \App\PostTypes\Core\CustomMetaBox\helpers\cmb_Meta_Box_field( $field_args );
				$field->render_field();
			}
		}
		echo '</table>';
	...
	public static function render_group( $args ) {
		...
		$args['count']   = 0;
		$field_group     = new \App\PostTypes\Core\CustomMetaBox\helpers\cmb_Meta_Box_field( $args );
		...
		<tr class="repeatable-grouping" data-iterator="'. $field_group->count() .'">
			<td>
				...
				foreach ( array_values( $field_group->args( 'fields' ) ) as $field_args ) {
					...
					$field = new \App\PostTypes\Core\CustomMetaBox\helpers\cmb_Meta_Box_field( $field_args, $field_group );
					$field->render_field();
				}
				echo '
					<tr>
...
// Handle oembed Ajax
add_action( 'wp_ajax_cmb_oembed_handler', array( '\App\PostTypes\Core\CustomMetaBox\helpers\cmb_Meta_Box_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_cmb_oembed_handler', array( '\App\PostTypes\Core\CustomMetaBox\helpers\cmb_Meta_Box_ajax', 'oembed_handler' ) );
 ...
 * @param  int    $object_id   Object ID
 * @param  string $object_type Type of object being saved. (e.g., post, user, or comment)
 * @return object              \App\PostTypes\Core\CustomMetaBox\helpers\cmb_Meta_Box_field object
 */
function cmb_get_field( $field_args, $object_id = 0, $object_type = 'post' ) {
	...
	return new \App\PostTypes\Core\CustomMetaBox\helpers\cmb_Meta_Box_field( $field_args );
}
